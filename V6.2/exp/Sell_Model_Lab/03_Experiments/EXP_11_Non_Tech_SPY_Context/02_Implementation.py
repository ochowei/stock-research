...
SPY_FEATURES = ['SPY_Gap_Pct', 'SPY_RSI_14', 'SPY_Dist_MA20', 'Market_Corr']
...
def load_tickers():
    ...
    with open(path, 'r') as f:
        raw = json.load(f)
    # Remove 'NASDAQ:' etc, and replace '.' with '-'
...
def fetch_sectors(tickers):
    ...
    # Reuse cache from previous experiments if possible
    # We'll look in EXP_07 or EXP_06
    exp07_cache = os.path.join(BASE_DIR, '..', 'EXP_07_Tech_Features', '03_Output', 'sector_map.json')
    exp06_cache = os.path.join(BASE_DIR, '..', 'EXP_06_Hyperparameter_Tuning', '03_Output', 'sector_map.json')
    ...
    if os.path.exists(exp07_cache):
        print(f"Loading sectors from EXP-07 cache ({exp07_cache})...")
        with open(exp07_cache, 'r') as f:
            return json.load(f)

    if os.path.exists(exp06_cache):
        print(f"Loading sectors from EXP-06 cache ({exp06_cache})...")
        with open(exp06_cache, 'r') as f:
            return json.load(f)
...
def fetch_data(tickers):
    benchmarks = ['SPY'] # We only need SPY for this experiment
    ...
    if isinstance(data.columns, pd.MultiIndex):
        ...
        # Fallback if single ticker (unlikely given the list)
...
def prepare_benchmark_features(spy_df):
    """Calculates features for SPY to be merged"""
    df = spy_df.copy()

    # Base Indicators
    df['Prev_Close'] = df['Close'].shift(1)

    # SPY Gap (Available at Open)
    df['SPY_Gap_Pct'] = (df['Open'] - df['Prev_Close']) / df['Prev_Close']

    # SPY T-1 Indicators
    df['SPY_RSI_14'] = ta.rsi(df['Close'], length=14).shift(1)
    ...
    df['SPY_Dist_MA20'] = (open_p / ma20_sim) - 1

    return df[['SPY_Gap_Pct', 'SPY_RSI_14', 'SPY_Dist_MA20', 'Close']] # Keep Close for correlation

def build_features(df, spy_df):
    """Feature Engineering - Base + Non-Tech (SPY)"""
    df = df.sort_index()
    ...
    try:
        ...
        spy_sub = spy_df.loc[common_idx]

        df_sub['SPY_Gap_Pct'] = spy_sub['SPY_Gap_Pct']
        df_sub['SPY_RSI_14'] = spy_sub['SPY_RSI_14']
        df_sub['SPY_Dist_MA20'] = spy_sub['SPY_Dist_MA20']

        # Correlation (Rolling 20 T-1)
        # Align closes
        aligned_close = pd.concat([df_sub['Close'], spy_sub['Close']], axis=1)
        aligned_close.columns = ['Stock_Close', 'SPY_Close']

        corr_series = aligned_close['Stock_Close'].rolling(20).corr(aligned_close['SPY_Close'])
        df_sub['Market_Corr'] = corr_series.shift(1)
    ...
    df_clean = df.dropna(subset=BASE_FEATURES + SPY_FEATURES)
    return df_clean
...
def main():
    print(f"=== EXP-11: Non-Tech Feature Augmentation (SPY Context) ===")

    tickers = load_tickers()
    sector_map = fetch_sectors(tickers)
    stock_raw, spy_raw = fetch_data(tickers)

    print("Preparing SPY features...")
    spy_feats = prepare_benchmark_features(spy_raw)
    ...
    for ticker, group in stock_raw.groupby('Ticker'):
        ...
        if sector == 'Technology':
            continue # We filter OUT Tech stocks

        df = group.set_index('Date').copy()
        df = df.dropna(subset=['Close'])
        if df.empty: continue

        feat_df = build_features(df, spy_feats)
    ...
    if not all_data:
        print("[Error] No signals found for Non-Tech Sector.")
        return

    full_df = pd.concat(all_data).sort_index()
    print(f"Total Valid Non-Tech Signals: {len(full_df)}")
    ...
    print("\n--- 1. Baseline Non-Tech Model (Base Features) ---")
    X_base = train_df[BASE_FEATURES]
    y_train = train_df['Label']
    w_train = train_df['Strategy_Ret'].abs() * 100

    model_base = tune_model(X_base, y_train, w_train, "NonTech_Base_Feats")

    # 2. Experiment: Base + SPY Features
    print("\n--- 2. Experiment Non-Tech Model (Base + SPY Features) ---")
    ALL_FEATS = BASE_FEATURES + SPY_FEATURES
    X_exp = train_df[ALL_FEATS]

    model_exp = tune_model(X_exp, y_train, w_train, "NonTech_SPY_Feats")
    ...
    # --- Results ---
    print("\n" + "="*60)
    print("RESULTS Comparison (Non-Tech Sector Only - Test Set 2024-2025)")
    print("="*60)
    print(f"{'Model':<25} | {'Win Rate':<10} | {'Avg Ret':<10} | {'Trades':<10}")
    print("-" * 60)
    print(f"{'Non-Tech Baseline':<25} | {base_win:.2%}    | {base_avg:.4f}    | {base_cnt}")
    print(f"{'Non-Tech + SPY Features':<25} | {exp_win:.2%}    | {exp_avg:.4f}    | {exp_cnt}")

    # Save Results
    res_data = [
        {'Model': 'NonTech_Baseline', 'Win_Rate': base_win, 'Avg_Ret': base_avg, 'Trades': base_cnt},
        {'Model': 'NonTech_SPY_Feats', 'Win_Rate': exp_win, 'Avg_Ret': exp_avg, 'Trades': exp_cnt},
    ]
    pd.DataFrame(res_data).to_csv(os.path.join(OUTPUT_DIR, 'performance_report.csv'), index=False)

    # Save Models
    joblib.dump(model_base, os.path.join(OUTPUT_DIR, 'model_non_tech_base.joblib'))
    joblib.dump(model_exp, os.path.join(OUTPUT_DIR, 'model_non_tech_exp.joblib'))
