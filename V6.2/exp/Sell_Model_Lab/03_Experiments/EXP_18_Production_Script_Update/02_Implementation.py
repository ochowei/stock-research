...
# Updated filenames to match actual artifacts
NON_TECH_MODEL_PATH = os.path.join(MODEL_DIR, 'v6.2.4_rc_non_tech_model.joblib')
TECH_MODEL_PATH = os.path.join(MODEL_DIR, 'v6.2.4_rc_tech_model.joblib')
...
def load_tickers():
    ...
    if not os.path.exists(path):
        # Fallback search for resource dir
        # Trying to find V6.2 root
        current = BASE_DIR
        while 'V6.2' in current:
            potential = os.path.join(current, 'resource', '2025_final_asset_pool.json')
            if os.path.exists(potential):
                path = potential
                break
            current = os.path.dirname(current)
            if current == os.path.dirname(current): # root
                break
...
def main():
    ...
    if not os.path.exists(NON_TECH_MODEL_PATH):
        print(f"Error: Model not found at {NON_TECH_MODEL_PATH}")
        return
    ...
    # Fetch 3 months of data to ensure enough for indicators
    ...
    res_df['Position_Size'] = res_df['Probability'].apply(get_position_size)
...
def main():
    ...
    try:
        # Run in the directory where the script is, so it finds the models
    ...
    # 3. Verify Output
    # Parse the stdout to find the output file name or search the dir
    # Filename format: daily_plan_{latest_date.date()}.csv
    # We can just list files in output dir and pick the csv

    csv_files = [f for f in os.listdir(OUTPUT_DIR) if f.startswith('daily_plan_') and f.endswith('.csv')]
    if not csv_files:
    ...
    print(f"Verifying {latest_csv}...")

    df = pd.read_csv(csv_path)

    if 'Position_Size' not in df.columns:
        print("FAIL: Position_Size column missing.")
        sys.exit(1)
    ...
    for _, row in df.iterrows():
        ...
        expected = 0.5
        if prob > 0.60: expected = 1.5
        elif prob > 0.55: expected = 1.0

        if abs(size - expected) > 1e-9:
            print(f"Mismatch: Prob {prob:.4f} -> Size {size}, Expected {expected}")
            errors += 1

    if errors == 0:
        print("SUCCESS: Position Sizing Logic Verified.")
    else:
        print(f"FAIL: {errors} logic errors found.")
        sys.exit(1)
