...
* **模型模擬 (v7.0 基礎版)：**
  * **進場 (T 日)：** 模擬在 T-1 日收盤後，以 **T-1 日收盤價 (Close)** 掛出一張**限價買單 (Limit Buy Order)**，此限價單在 T 日的正常交易時段 (RTH) 有效。
    * **成交條件：** 如果 T 日的**日線最低價 (Low) <= T-1 日收盤價**，則訂單成交 (`Fill_Status = 'FILLED'`)。
    * **失敗條件：** 如果 T 日的**日線最低價 > T-1 日收盤價**，則訂單未成交 (`Fill_Status = 'NO_FILL'`)，意味著價格未回調至掛單價位。
  * **出場 (T+1 日)：** 2.2.2 節中的「T+1 日的開盤價」模擬在 T+1 開盤時以市價單出場。
* **(新增) 未來可擴展的模擬方向：**
...
*   **進場價格 (p)**：`p = T-1 Close` (T-1 日的日線收盤價)。這是限價單的掛單價格。
*   **波動率單位 (vol)**：`vol = ATR(14)` (T-1 日的 14 天日線 ATR **非標準化**值)。
*   **T 日最低價 (T\_Low)**：`T_Low = T-day Low`。用於判斷限價單是否成交。
*   **出場價格 ($p_{exit}$)**：`p_exit = T+1 Open`。

**2.2.2. Y 標籤定義 (回歸)**

*   **情境 (A) - 已成交 (FILLED):**
    *   **成交條件:** `T_Low <= p` (T 日的最低價觸及或低於 T-1 的收盤價)。
    *   **結算價格 ($p_{exit}$):** T+1 日的**開盤價**。
    *   **Y (標籤) =** `(p_exit - p) / vol`
        *   *(範例：若 T+1 開盤價為* $p+1.5 \times vol$*，則 Y = 1.5)*
        *   *(範例：若 T+1 開盤價為* $p-0.8 \times vol$*，則 Y = -0.8)*
*   **情境 (B) - 未成交 (NO\_FILL):**
    *   **失敗條件:** `T_Low > p` (T 日的最低價從未回調至 T-1 的收盤價)。
    *   **Y (標籤) = `NaN`** (此樣本將在數據清理階段被移除，見 3.1 節)。
