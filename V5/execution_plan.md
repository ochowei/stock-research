### **📈 專案 V5-ML 執行計畫：跨截面均值回歸與黑天鵝防禦系統 (修訂版)**
...
#### **步驟 0-1：原始數據下載 (Data Download)**

此步驟僅負責從網路獲取數據並暫存，不進行複雜處理，以確保數據源的純淨並減少重複下載的風險。

* **目標：** 下載並保存 yfinance 原始數據。
* **執行：**
  1. **資產池更新：** 鎖定高流動性標的 (S&P 100 / Nasdaq 100)。
  2. **下載執行：**
     * 呼叫 yfinance 下載個股 OHLCV (Daily & 60m)。
     * 下載宏觀市場指標 (SPY, QQQ, IWO, VTI, ^VIX, ^TNX 等)。
  3. **暫存：** 將下載回來的原始 DataFrame (Raw Dictionary or Pickle) 存入 `data/temp_raw/` 目錄。
* **產出檔案：**
...
此步驟讀取暫存的原始數據，進行格式標準化（Stacking, MultiIndex），產出下游模型所需的 Parquet 檔案。

* **目標：** 製作標準化的 `universe_daily.parquet` 與 `market_indicators.parquet`。
* **執行：**
  1. **讀取暫存檔：** 載入步驟 0-1 的數據。
  2. **格式化：**
     * 處理 MultiIndex (timestamp, symbol)。
     * 處理欄位名稱與型別。
  3. **儲存：** 寫入正式的 Parquet 檔案。
* **產出檔案：**
...
* **執行：**
  1. **讀取數據：** 載入 `universe_daily.parquet` 與 `market_indicators.parquet`。
  2. **市場特徵 (For L1):** * 計算 **Proxy ETF 特徵:**
       * **IWO_Volatility:** IWO 的滾動已實現波動率。
       * **SPY_IWO_Divergence:** SPY 與 IWO 之間的滾動回報差異。
     * 計算 **波動率特徵:** VIX 的變動率、SPY 的 Realized Volatility。
  3. **個股特徵 (For L2 & L3):** * **均值回歸因子:** RSI(2), Bollinger Band %B, Distance to SMA200。
...
* **執行：**
  1. **HMM 模型訓練:**
     * 使用 market_features_L0 (含 Proxy ETF 特徵) 訓練 Gaussian HMM。
...
* **執行：**
  ...
  3. **L3 模型訓練 (Optimization):**
     * 輸入：stock_features_L0 + regime_signals (將市場狀態作為上下文特徵)。
...
本計畫將生成以下主要的 Python 腳本：

1. `00_download_data_v5.py`: 涵蓋步驟 0-1，負責從 yfinance 下載原始數據並暫存。
2. `01_format_data_v5.py`: 涵蓋步驟 0-2，負責將暫存數據格式化為 Parquet。
3. `02_build_features_l0_v5.py`: 涵蓋步驟 1，負責計算 L0 特徵。
